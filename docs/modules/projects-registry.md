...
- 2026-09-02 追加（Round 1 / sessions-tg-relay）：§10「本轮扩展」——纯函数区新增
  `sessions` 承载；§2.1 的「零改动」在本轮被部分 supersede；锁语义 §3/§4 零触碰。

## 10. 本轮扩展（sessions-tg-relay, Round 1，追加不改写）

> 追加章节，**不改写**上面任何已冻结契约。唯一权威描述见
> docs/modules/sessions-relay.md；本节只声明与 §2.1/§7 的边界关系。

- **§2.1「纯函数区零改动」在本轮 supersede 的范围**（sessions 相关，其余仍零改动）：
  - `RegistryEntry` 增加可选字段 `sessions?: SessionRecord[]`（新增类型 `SessionRecord`，
    冻结定义见 docs/modules/sessions-relay.md §2）；
  - `parseRegistry` 白名单扩展：保留并严格校验 sessions（非数组/记录字段类型不符 →
    丢弃该字段/该记录，**不抛错**）；条目缺 path 整体返回 undefined 的既有行为不变；
  - `serializeRegistry` 零改动（JSON.stringify 天然携带 sessions）；
  - 新增 3 个纯函数：`appendSessionRecord` / `markSessionResolved` / `markSessionSent`
    （签名、幂等与 undefined 语义冻结见 docs/modules/sessions-relay.md §4）。
- **§3/§4 锁语义零触碰**：`mutate()` 流程、参数常量、错误语义、无 touch、serialized()
  保留、锁文件路径全部不变。sessions 的追加/置位全部经既有 `mutate(fn)` 消费：
  fn 返回 `undefined` → 不写盘不抛错（§4 步骤 2）；幂等（fn 返回原引用）→ 无写盘仅刷缓存
  （§4 步骤 3）。sessions 纯函数返回原引用/undefined 即走这两条既有路径，无需改 mutate。
- **§7 验证门 supersede**：`behavior.test.mjs` 断言由 API-001/002/003 改为 API-001~005
  （新语义见 sessions-relay.md §8），并新增 `tests/registry-sessions.test.mjs`
  （REG-101）与 `tests/sessions-poller.test.mjs`（API-006）；LOCK-001~005 不变。
- **新增测试归属**：sessions 纯函数测试独立成 `tests/registry-sessions.test.mjs`，
  **不改动** `tests/registry-concurrency.test.mjs`（LOCK-001~005 仍为锁契约独占验证门）。